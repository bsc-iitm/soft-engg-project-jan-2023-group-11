<template>
  <div class="component-6">
    <form>
      <div class="flex-wrapper-two">
        <p class="python">
          ...
          <select id="category" name="category" v-model="query.category">
    ...
    <div class="flex-wrapper-one">
      <p class="post-a-query">Post A Query ???</p>
      <div class="minus">
...
export default {
  ...
  methods: {
    postQuery() {
      ...
      axios
        .post(
          `/student/create_ticket/${userID}/${this.query.title}/${this.query.category}/${this.query.description}`
        )
        .then((res) => {
          console.log(res);
          this.$store.state.queries = res.data.queries;
          this.$store.state.createPostModal =
            !this.$store.state.createPostModal;
        })
        .catch((err) => {
          console.log(err);
    ...
    closemodal() {
      this.$store.state.createPostModal = false;
    },
  },
};
</script>
